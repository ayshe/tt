...
const togglePlayer = (player) => {
    ...
    if (playerarray.length == 2) {
        ...
        if (playerarray[0].score == null || playerarray[1].score == null) {
            matchup(playerarray[0].name, playerarray[1].name, null, null, onwin, onupset);
            return;
        }
    ...
    if (playerarray.length == 4) {
        ...
        if (playerarray[0].score == null || playerarray[1].score == null || playerarray[2].score == null || playerarray[3].score == null) {
            matchup(playerarray[0].name + " / " + playerarray[1].name, playerarray[2].name + " / " + playerarray[3].name, null, null, onwin, onupset);
            return;
        }
...
const draw = (data) => {
    ...
    data.forEach(function (player) {
        var panel = jQuery(document.createElement('div')).attr('class', 'col l2 s4 card-panel center-align ' + (player.score == null ? 'yellow' : 'blue') + ' lighten-5 player').append(jQuery(document.createElement('h6')).text(player.name + (player.score == null ? '' : " (" + player.score + ")")));
